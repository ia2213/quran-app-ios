...
class AppState extends ChangeNotifier {
  ...
  String? _kidsModeType;
  int _kidsGateStep = 0;
  bool _kidsMathChallengeActive = false;
  int _kidsMathAnswer = 0;

  bool get isKidsMode => _isKidsMode;
  String? get kidsModeType => _kidsModeType;
  bool get isKidsModeExitPending => _kidsModeType != null && !_isKidsMode;
  int get kidsGateStep => _kidsGateStep;
  bool get kidsMathChallengeActive => _kidsMathChallengeActive;
  int get kidsMathAnswer => _kidsMathAnswer;

  void setKidsMode(bool val, {String? modeType}) async {
    _isKidsMode = val;
    if (val && modeType != null) {
      _kidsModeType = modeType;
      _kidsGateStep = 0;
      _kidsMathChallengeActive = false;
    } else if (!val) {
      _kidsModeType = null;
      _kidsGateStep = 0;
    }
    ...
    await prefs.setBool('isKidsMode', val);
    if (modeType != null) await prefs.setString('kidsModeType', modeType);
  }

  void startKidsGate({bool useMath = false}) {
    _kidsMathChallengeActive = useMath;
    _kidsGateStep = 0;
    if (useMath) {
      // Generate simple math challenge for kids
      final a = 2 + (DateTime.now().millisecond % 5); // 2-6
      final b = 1 + (DateTime.now().millisecond % 4); // 1-4
      _kidsMathAnswer = a + b;
    }
    notifyListeners();
  }

  bool validateKidsGateSequence(int digit) {
    final expected = [2, 4, 6, 8, 10];
    if (_kidsGateStep >= expected.length) return true;
    if (digit == expected[_kidsGateStep]) {
      _kidsGateStep++;
      notifyListeners();
      return _kidsGateStep >= expected.length;
    }
    _kidsGateStep = 0;
    notifyListeners();
    return false;
  ...
  bool validateKidsMathChallenge(int answer) {
    if (answer == _kidsMathAnswer) {
      _kidsMathChallengeActive = false;
      _isKidsMode = true;
      notifyListeners();
      return true;
  ...
  void loadKidsModeState() async {
    final prefs = await SharedPreferences.getInstance();
    _isKidsMode = prefs.getBool('isKidsMode') ?? false;
    _kidsModeType = prefs.getString('kidsModeType');
    notifyListeners();
  }
...
class QuranApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => AppState()..loadSurahs()..loadTheme()..loadKidsModeState(),
      child: Consumer<AppState>(
        builder: (context, appState, _) {
          return MaterialApp(
            ...
            home: appState.isKidsMode ? const KidsModeScreen() : const AdultHomeScreen(),
          );
        },
      ),
    );
...
class AdultHomeScreen extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        ...
        GridView.count(
          ...
          children: [
            for (int i = 1; i <= 9; i++)
              _numberButton(context, i, appState, setState, isDark),
            const SizedBox(height: 8),
            _numberButton(context, 0, appState, setState, isDark),
            Container(
              width: 60,
              height: 60,
              decoration: BoxDecoration(
  ...
  Widget _numberButton(BuildContext context, int digit, AppState appState, StateSetter setState, bool isDark) {
    final colorScheme = Theme.of(context).colorScheme;
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _lessonCard('Ghunnah', Icons.volume_up, 'La nasalisation'),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            icon: Icon(actionIcon, color: kKidsPrimary),
            onPressed: () {},
          ),
        ],
      ),
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _adhkarChip('Soir', Icons.nights_stay),
